...
// And every 2s we poll state.db for status flips on watched ids and emit
...
export class SessionsWatcher {
  ...
  async start(): Promise<void> {
    ...
    // Status-flip poller. Runs every 2s and emits status_change events
    // for any watched session whose state.db row's status doesn't match
    // our cached SessionInfo.status.
